...
namespace PoRacer.CreatureRace
{
    ...
    /// position of the racers along the track and stays centred on the lanes. Reads the model
    /// in LateUpdate because a camera has to move every frame; it decides nothing.
    /// </summary>
    [DisallowMultipleComponent]
    [RequireComponent(typeof(Camera))]
    public sealed class CreatureRaceCameraView : MonoBehaviour
    {
        ...
        [Inject]
        public void Construct(CreatureRaceModel model)
        {
            _model = model;
        }
        ...
        private Vector3 Focus()
        {
            ...
            // Follow the racers that are actually racing: a NO BRAIN racer stands on the start
        ...
        private bool TryAverageNose(bool brainedOnly, out float noseZ)
        {
            ...
            for (int lane = 0; lane < _model.Racers.Count; lane++)
            {
                ...
                if (racer.Status == CreatureRacerStatus.Failed || (brainedOnly && !racer.BrainReady))
                {
                    continue;
                }
