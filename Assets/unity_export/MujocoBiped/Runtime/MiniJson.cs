...
namespace MujocoBiped
{
    ...
    ///
    /// Unity's JsonUtility cannot do what these files need: it cannot represent a
    /// nullable nested object (a rig link has no "joint" when it is the articulation
    /// root, and JsonUtility would silently hand back a default-constructed one), and it
    /// cannot read the jagged float arrays that carry the recorded observations and
    /// actions. It lives in the runtime assembly only so the PlayMode test
    ...
    /// parses JSON - it reads the generated MujocoBipedRigAsset.
    ///
