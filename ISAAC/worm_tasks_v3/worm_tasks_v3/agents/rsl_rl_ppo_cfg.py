"""RSL-RL PPO runner cfg for Worm5 (Isaac Lab 3 / Newton task), exactly WORM_SPEC.md "PPO (both)" -
identical to the 2.3 task's cfg (same rsl-rl-lib 5.0.1).
...
norm 1.0. The wall-clock budget (30 min) is enforced by ISAAC/scripts/train_worm_v3.py, so
...
class WormFlatPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    ...
    save_interval = 50
    experiment_name = "worm5_newton"  # ISAAC/logs/rsl_rl_v3/worm5_newton
