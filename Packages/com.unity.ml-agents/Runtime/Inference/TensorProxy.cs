...
namespace Unity.MLAgents.Inference
{
    ...
    internal class TensorProxy
    {
        ...
        void Dispose()
        {
            // PORACER PATCH (2026-08-29) - see Packages/com.unity.ml-agents/PORACER_PATCHES.md
            //
            // This ran on the finalizer thread and dereferenced `data.dataOnBackend`
            // with no null check, while the very next line used `data?.` - so null was
            // already expected here. ModelRunner.FetchSentisOutputs wraps worker-owned
            // tensors from PeekOutput() in fresh TensorProxy objects on every decision
            // and drops the previous ones; once the worker releases that backing store,
            // `data` or `data.dataOnBackend` is null and the finalizer threw.
            //
            // Measured on a Pixel 9 Pro: 3,553 NullReferenceExceptions in ~2 minutes of
            // play, about 30 per second, every one of them logged with a stack trace.
            // The guarded body is a no-op for this project anyway (SentisModelInfo
            // builds its Worker with DeviceType.CPU), so this only removes waste.
