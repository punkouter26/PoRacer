# Unattended 5-hour all-creatures training run with a timestamped run-id.
# Usage: .\scripts\train_all_5h.ps1 [-ConfigPath Config\AllLoco5h01.yaml] [-Hours 5]
# max_steps in the config (2M) is calibrated to finish near the 5 h mark; the
...
param(
    [string]$ConfigPath = "Config\AllLoco5h01.yaml",
...
$RunId = "all5h_" + (Get-Date -Format "yyyyMMdd_HHmm")
