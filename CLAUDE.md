...
- Prefab: `Assets/Agents/Fido_v01/Fido.prefab`, regenerated by
  `Editor_BuildFidoPrefab.Build()` whenever `creature.xml` changes.
...
`PoRacer`, `IsaacBox`, `IsaacH1` and `MuJoCo Creature` menus) was stripped so that all
editor work goes through MCP / the Unity CLI. **The classes and methods are unchanged
and still public** — only the menu-bar UI is gone. Do not re-add a `[MenuItem]`; add
the method and call it.

```
unity command eval --code "PoRacer.EditorTools.Editor_ConfigureAndroidRelease.Apply()"
```

Namespaces are not uniform, and guessing costs a round trip: `Assets/Scripts/Editor/`
is split between `PoRacer.EditorTools` and `PoRacer.Editor`, while the ports use
`IsaacBox.EditorTools`, `IsaacH1.EditorTools` and `MujocoBiped.EditorTools`. Check the
file before calling.
