...
* **Telemetry & Shutdown:** Output telemetry via HTTP *and* `StatsRecorder`. **Always start TensorBoard (`.venv\Scripts\tensorboard.exe --logdir results --port 6006`) whenever launching `mlagents-learn` — no training run without it.** Kill TensorBoard before `--force` wipes (Windows file handles will silently fail the wipe). Tear down in strict order: trainer → envs → TensorBoard.
