...
* **Telemetry & Shutdown:** Output telemetry via HTTP *and* `StatsRecorder`. Kill TensorBoard before `--force` wipes (Windows file handles will silently fail the wipe). Tear down in strict order: trainer → envs → TensorBoard.
